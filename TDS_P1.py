...
def main():
    ...
    users_df.to_csv('users1.csv', index=False)
    ...
    repos_df.to_csv('repositories1.csv', index=False)
...
1. `users1.csv`: Contains information about {len(users)} GitHub users in London with over 500 followers
2. `repositories1.csv`: Contains information about {len(all_repos)} public repositories from these users
